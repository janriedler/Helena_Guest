<template>
  <div style="display: flex; flex-direction: column;">
    <div  v-if="!profil" style="display: flex; flex-direction: row; justify-content: center; align-items: center; margin: 40px">
      <div>
        <el-input
            v-model="search"
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <el-button v-if="column.label === 'Name'" @click.native="callId(scope.row.id)">{{ scope.row.name }}</el-button>
            <span v-else>{{ scope.row[column.prop] }}</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
    <div v-if="profil" style="display: flex; flex-direction: row; justify-content: center; align-items: center;">
    ...
    <div v-if="profil" class="table">
      <el-form ref="form"  label-width="120px" style="margin-top: 5%">
        ...
        <el-form-item label="Check-In:" style="border:1px solid #000066">
          <el-date-picker v-if="edit" v-model="checkIn" type="date" placeholder="Check-In" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="float: left">
          </el-date-picker>
          <span v-else style="float: left">{{checkIn}}</span>
        </el-form-item>
        <el-form-item label="Check-Out:" style="border:1px solid #000066">
          <el-date-picker v-if="edit" v-model="checkOut" type="date" placeholder="Check-In" format="yyyy-MM-dd" value-format="yyyy-MM-dd" style="float: left">
          </el-date-picker>
  ...
  methods: {
          ...
          .then((response) => {
            ...
            for (let j = 0; j < unprocessed.length; j++) {
              const obj = {
                ...
                'gezahlt/preis': unprocessed[j]['gezahlt'] + '/' + unprocessed[j]['preis'],
              }
              rows.push(obj)
            }
    ...
    doSearch() {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        ...
        let cellSonst = JSON.parse(JSON.stringify(this.tableData[i]['sonstiges']))
        if (cellName.toLowerCase().includes(this.search.toLowerCase())) {
          filterRows.push(this.tableData[i])
        } else if (cellEmail.toLowerCase().includes(this.search.toLowerCase())) {
            filterRows.push(this.tableData[i])
          }
         else if (cellSonst.toLowerCase().includes(this.search.toLowerCase())) {
            filterRows.push(this.tableData[i])
          }
    ...
    submit() {
      let all = []
      all.push(this.id, this.name, this.email, this.checkIn, this.checkOut, this.sonstiges, this.raum, this.preis, this.gezahlt)
      console.log("Haaalalloo" + all);
      axios
          .get('/edit', { params: {id: this.id, name: this.name, email:this.email, checkIn: this.checkIn,
